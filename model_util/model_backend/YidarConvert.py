...
#        assert len(test_wavs) == 1
        ...
        for idx, wav in enumerate(test_wavs):
            ...
            wav_id = wav_name.split('.')[0]
            try:
                out_name = config.trg_name
            except:
                out_name = f'{wav_id}-vcto-{test_loader.trg_spk}.wav'

            full_path = join(config.convert_dir, str(config.resume_iters), out_name)
            paths_ret.append(full_path)
            librosa.output.write_wav(full_path, wav_transformed, sampling_rate)
            if [True, False][0]:
                wav_cpsyn = world_speech_synthesis(f0=f0, coded_sp=coded_sp, 
                                                ap=ap, fs=sampling_rate, frame_period=frame_period)
                librosa.output.write_wav(join(config.convert_dir, str(config.resume_iters), f'cpsyn-{wav_name}'), wav_cpsyn, sampling_rate)
